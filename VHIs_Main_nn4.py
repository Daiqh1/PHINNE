...
def main():
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        sumHHHHV, _ = metaPath_Hsim_HV(fused_simHr, sumHHHV, 4)
        _, maxHHHHV = metaPath_Hsim_HV(fused_simHr, maxHHHV, 4)

        sumHHHVV, _ = metaPath_HV_Vsim(fused_simVr, sumHHHV, 4)
        _, maxHHHVV = metaPath_HV_Vsim(fused_simVr, maxHHHV, 4)

        sumHHVVV, _ = metaPath_Hsim_HV(fused_simHr, sumHVVV, 4)
        _, maxHHVVV = metaPath_Hsim_HV(fused_simHr, maxHVVV, 4)

        sumHVVVV, _ = metaPath_HV_Vsim(fused_simVr, sumHVVV, 4)
        _, maxHVVVV = metaPath_HV_Vsim(fused_simVr, maxHVVV, 4)

        sumHHVHV, _ = metaPath_Hsim_HV(fused_simHr, sumHVHV, 4)
        _, maxHHVHV = metaPath_Hsim_HV(fused_simHr, maxHVHV, 4)

        sumHVHVV, _ = metaPath_HV_Vsim(fused_simVr, sumHVHV, 4)
        _, maxHVHVV = metaPath_HV_Vsim(fused_simVr, maxHVHV, 4)

        sumHVVHV, maxHVVHV = metaPath_HVVHV(HrVr_train, fused_simVr)
        sumHVHHV, maxHVHHV = metaPath_HVHHV(HrVr_train, fused_simHr)
        ...
        sumHHHHV2, _ = metaPath_Hsim_HV(cos_simHH, sumHHHV2, 4)
        _, maxHHHHV2 = metaPath_Hsim_HV(cos_simHH, maxHHHV2, 4)

        sumHHHVV2, _ = metaPath_HV_Vsim(cos_simVV, sumHHHV2, 4)
        _, maxHHHVV2 = metaPath_HV_Vsim(cos_simVV, maxHHHV2, 4)

        sumHHVVV2, _ = metaPath_Hsim_HV(cos_simHH, sumHVVV2, 4)
        _, maxHHVVV2 = metaPath_Hsim_HV(cos_simHH, maxHVVV2, 4)

        sumHVVVV2, _ = metaPath_HV_Vsim(cos_simVV, sumHVVV2, 4)
        _, maxHVVVV2 = metaPath_HV_Vsim(cos_simVV, maxHVVV2, 4)

        sumHHVHV2, _ = metaPath_Hsim_HV(cos_simHH, sumHVHV2, 4)
        _, maxHHVHV2 = metaPath_Hsim_HV(cos_simHH, maxHVHV2, 4)

        sumHVHVV2, _ = metaPath_HV_Vsim(cos_simVV, sumHVHV2, 4)
        _, maxHVHVV2 = metaPath_HV_Vsim(cos_simVV, maxHVHV2, 4)

        sumHVVHV2, maxHVVHV2 = metaPath_HVVHV(HrVr_train, cos_simVV)
        sumHVHHV2, maxHVHHV2 = metaPath_HVHHV(HrVr_train, cos_simHH)
        ...
        for i in range(len(allH)):
            for j in range(len(allV)):
                pair_scores = (allH[i], allV[j], \
                               ...
                               sumHHHHV[i][j], sumHHHVV[i][j], sumHHVVV[i][j], sumHVVVV[i][j],
                               sumHHVHV[i][j], sumHVHVV[i][j], sumHVVHV[i][j], sumHVHHV[i][j],

                               maxHHHHV[i][j], maxHHHVV[i][j], maxHHVVV[i][j], maxHVVVV[i][j],
                               maxHHVHV[i][j], maxHVHVV[i][j], maxHVVHV[i][j], maxHVHHV[i][j],
                               # path scores from G2

                               sumHHHHV2[i][j], sumHHHVV2[i][j], sumHHVVV2[i][j], sumHVVVV2[i][j],
                               sumHHVHV2[i][j], sumHVHVV2[i][j], sumHVVHV2[i][j], sumHVHHV2[i][j],

                               maxHHHHV2[i][j], maxHHHVV2[i][j], maxHHVVV2[i][j], maxHVVVV2[i][j],
                               maxHHVHV2[i][j], maxHVHVV2[i][j], maxHVVHV2[i][j], maxHVHHV2[i][j])
        ...
        for i in range(len(HV_score)):
            ...
            edgeScore = HV_score[i][2], HV_score[i][3], HV_score[i][4], HV_score[i][5], \
                        ...
                        HV_score[i][20], HV_score[i][21], HV_score[i][22],HV_score[i][23],\
                        HV_score[i][26],HV_score[i][27],HV_score[i][28],HV_score[i][29],\
                        \
                        HV_score[i][34],HV_score[i][35],HV_score[i][36],HV_score[i][37],\
                        \
                        HV_score[i][42],HV_score[i][43],HV_score[i][44],HV_score[i][45],\
                        \
                        HV_score[i][50],HV_score[i][51],HV_score[i][52],HV_score[i][53]
    ...
    hv_df = pd.DataFrame(all_hv_PredictedScore,
                         columns=['HV_pair', 'Predicted_score_class1', 'Predicted_Class', 'Actual_Class'])
