...
from Path_Scores_functions import *
...
def main():
    ...
    if not os.path.exists(novel_DT_file):
        file = open(novel_DT_file, 'w')
        file.close()
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        sumDDDDT,_=metaPath_Dsim_DT(fused_simDr,sumDDDT,4)
        _,maxDDDDT=metaPath_Dsim_DT(fused_simDr,maxDDDT,4)

        sumDDDTT,_=metaPath_DT_Tsim(fused_simTr,sumDDDT,4)
        _,maxDDDTT=metaPath_DT_Tsim(fused_simTr,maxDDDT,4)

        sumDDTTT,_=metaPath_Dsim_DT(fused_simDr,sumDTTT,4)
        _,maxDDTTT=metaPath_Dsim_DT(fused_simDr,maxDTTT,4)

        sumDTTTT,_=metaPath_DT_Tsim(fused_simTr,sumDTTT,4)
        _,maxDTTTT=metaPath_DT_Tsim(fused_simTr,maxDTTT,4)

        sumDDTDT,_=metaPath_Dsim_DT(fused_simDr,sumDTDT,4)
        _,maxDDTDT=metaPath_Dsim_DT(fused_simDr,maxDTDT,4)

        sumDTDTT,_=metaPath_DT_Tsim(fused_simTr,sumDTDT,4)
        _,maxDTDTT=metaPath_DT_Tsim(fused_simTr,maxDTDT,4)

        sumDTTDT,maxDTTDT=metaPath_DTTDT(DrTr_train,fused_simTr)
        sumDTDDT,maxDTDDT=metaPath_DTDDT(DrTr_train,fused_simDr)
        ...
        sumDDDDT2,_=metaPath_Dsim_DT(cos_simDD,sumDDDT2,4)
        _,maxDDDDT2=metaPath_Dsim_DT(cos_simDD,maxDDDT2,4)

        sumDDDTT2,_=metaPath_DT_Tsim(cos_simTT,sumDDDT2,4)
        _,maxDDDTT2=metaPath_DT_Tsim(cos_simTT,maxDDDT2,4)

        sumDDTTT2,_=metaPath_Dsim_DT(cos_simDD,sumDTTT2,4)
        _,maxDDTTT2=metaPath_Dsim_DT(cos_simDD,maxDTTT2,4)

        sumDTTTT2,_=metaPath_DT_Tsim(cos_simTT,sumDTTT2,4)
        _,maxDTTTT2=metaPath_DT_Tsim(cos_simTT,maxDTTT2,4)

        sumDDTDT2,_=metaPath_Dsim_DT(cos_simDD,sumDTDT2,4)
        _,maxDDTDT2=metaPath_Dsim_DT(cos_simDD,maxDTDT2,4)

        sumDTDTT2,_=metaPath_DT_Tsim(cos_simTT,sumDTDT2,4)
        _,maxDTDTT2=metaPath_DT_Tsim(cos_simTT,maxDTDT2,4)

        sumDTTDT2,maxDTTDT2=metaPath_DTTDT(DrTr_train,cos_simTT)
        sumDTDDT2,maxDTDDT2=metaPath_DTDDT(DrTr_train,cos_simDD)
        # ==============================================================================
        ...
        for i in range(len(allD)):
            for j in range(len(allT)):
                pair_scores = (allD[i], allT[j],
                               ...
                               sumDDDDT[i][j], sumDDDTT[i][j], sumDDTTT[i][j], sumDTTTT[i][j],
                               sumDDTDT[i][j], sumDTDTT[i][j], sumDTTDT[i][j], sumDTDDT[i][j],

                               maxDDDDT[i][j], maxDDDTT[i][j], maxDDTTT[i][j], maxDTTTT[i][j],
                               maxDDTDT[i][j], maxDTDTT[i][j], maxDTTDT[i][j], maxDTDDT[i][j],
                               # path scores from G2

                               sumDDDDT2[i][j], sumDDDTT2[i][j], sumDDTTT2[i][j], sumDTTTT2[i][j],
                               sumDDTDT2[i][j], sumDTDTT2[i][j], sumDTTDT2[i][j], sumDTDDT2[i][j],

                               maxDDDDT2[i][j], maxDDDTT2[i][j], maxDDTTT2[i][j], maxDTTTT2[i][j],
                               maxDDTDT2[i][j], maxDTDTT2[i][j], maxDTTDT2[i][j], maxDTDDT2[i][j])
        ...
        for i in range(len(DT_score)):
            dr = DT_score[i][0]
            tr = DT_score[i][1]
            # DT_score[i][6],DT_score[i][7],\
            # DT_score[i][12],DT_score[i][13],\
            # DT_score[i][18],DT_score[i][19],
            # DT_score[i][24],DT_score[i][25]
            # DT_score[i][30],DT_score[i][31],DT_score[i][32], DT_score[i][33],
            # DT_score[i][38],DT_score[i][39],DT_score[i][40], DT_score[i][41],
            # DT_score[i][46],DT_score[i][47],DT_score[i][48], DT_score[i][49],
            # DT_score[i][54],DT_score[i][55],DT_score[i][56], DT_score[i][57]
            edgeScore = DT_score[i][2], DT_score[i][3], DT_score[i][4], DT_score[i][5],\
                        ...
                        DT_score[i][20], DT_score[i][21], DT_score[i][22], DT_score[i][23],\
                        DT_score[i][26],DT_score[i][27],DT_score[i][28],DT_score[i][29],\
                        \
                        DT_score[i][34],DT_score[i][35],DT_score[i][36],DT_score[i][37],\
                        \
                        DT_score[i][42],DT_score[i][43],DT_score[i][44],DT_score[i][45],\
                        \
                        DT_score[i][50],DT_score[i][51],DT_score[i][52],DT_score[i][53]
    ...
    dt_df = pd.DataFrame(all_dt_PredictedScore,
                         columns=['DT_pair', 'Predicted_score_class1', 'Predicted_Class', 'Actual_Class'])
